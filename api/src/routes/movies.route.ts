...
MoviesRoutes.route('/').get(async function (_req: Request, res: Response) {
    //console.log(' : ', req);
    const url = `${environment.urlMoviedb}/discover/movie?primary_release_date.gte=${from}&primary_release_date.lte=${to}&api_key=${environment.apikey}&language=es`;
...
MoviesRoutes.route('/:id').get(async function (req: Request, res: Response) {
    ...
    const url = `${environment.urlMoviedb}/movie/${id}?api_key=${environment.apikey}&language=es`;
    let promise = new Promise(async function(resolve, reject) {
        await axios.get(url)
            .then(function (response) {
                // handle success
...
MoviesRoutes.route('/:moviename').get(async function (req: Request, res: Response) {
